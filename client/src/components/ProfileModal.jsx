            ...
            <div className='bg-white rounded-lg shadow p-6 '>
                  ...
                  <form className='space-y-4' onSubmit={handleSaveProfile}>
                    ...
                    <div className='flex flex-col items-start gap-3'>
                        ...
                        <input hidden 
                         type="file" accept="image/*" id="profile_picture" className='w-full p-3 border border-gray-200 rounded-lg'
                        onChange={(e)=>setEditForm({...editForm, profile_picture: e.target.files[0]})}/>
                        <div className='group/profile relative'>
                        <img src={editForm.profile_picture ? URL.createObjectURL(editForm.profile_picture) : user.profile_picture} alt="Profile Picture" className='w-24 h-24 rounded-full object-cover mt-2' />
                   ...
                   <div className='flex flex-col items-start gap-3'>
                    <label htmlFor='cover_photo' className='block text-sm font-medium text-gray-700 mb-1'>
                        Cover Photo
                         <input hidden 
                         type="file" accept="image/*" id="cover_photo" className='w-full p-3 border border-gray-200 rounded-lg'
                        onChange={(e)=>setEditForm({...editForm, cover_photo: e.target.files[0]})}/>
                        <div className='group/cover relative'>
                            <img src={editForm.cover_photo ? URL.createObjectURL(editForm.cover_photo) : user.cover_photo} alt="Cover Photo" className='w-80 h-40 rounded-lg bg-gradient-to-r from-indigo-200 via-purple-200 to-pink-200 object-cover mt-2' />
