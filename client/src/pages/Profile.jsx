                  ...
                  <div className='flex flex-wrap mt-6 max-w-6xl '>
                    {
                     posts.filter((post)=>post.image_urls.length > 0).map((post)=>(
                      ...
                      {post.image_urls.map((image, index )=>(
                        ...
                        <p className='absolute bottom-0 right text-xs p-1 px-3 backdrop-blur-xl text-white opacity-0 group-hover:opacity-100 transition duration-300'>Posted {moment(post.created_at).fromNow()}</p>
                        </Link>
                      ))}
                      </>
                     ))
